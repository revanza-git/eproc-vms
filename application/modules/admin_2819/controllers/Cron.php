		...
		*/
		...
		foreach ($query as $key => $value) {
			...
			JOIN ms_vendor_pic d ON a.id = d.id_vendor WHERE a.id = ?";
							$data_vendor = $this->db->query($data_vendor, array($_queryDocument
			...
			=> $valueDocument){
										$selectTransaction = $this->db->where

			('id_vendor', $valueDocument['id_vendor'])->where('id_dpt_type', $valueDocument['id_dpt_type'])->where('end_date IS NULL')->get('tr_dpt');

										$queryTransaction = $this->db->where

			('id_vendor', $valueDocument['id_vendor'])->where('id_dpt_type', $valueDocument['id_dpt_type'])->where('end_date IS NULL')->update('tr_dpt', array('status'=>2,'end_date'=>date('Y-m-d H:i:s')));
			...
			['id_vendor'],
										...
										if($queryDPTCheck->num_rows()==0){
											$this->db->where('id', $valueDocument

			['id_vendor'])->update('ms_vendor', array('vendor_status'=> 1));
										}
			...
			('id_vendor', $valueDocument['id_vendor'])->where('end_date IS NULL')->update('tr_dpt', array

			('status'=>2,'end_date'=>date('Y-m-d H:i:s')));
										$selectDPT = $this->db->query("SELECT * FROM 

			tr_dpt WHERE id_vendor = ? GROUP BY id_dpt_type", array($valueDocument['id_vendor']))->result_array();
			...
			diperhatikan :
							...
							$this->email->to($data_vendor

			['vendor_email']); 
							$this->email->cc($data_vendor['pic_email']); 
							$this->email->bcc('hanafi@nusantararegas.com'); 
$this->email->bcc('logistik@nusantararegas.com'); 
							$this->email->bcc('muarifgustiar@gmail.com'); 
							$this->email->subject('Notifikasi Masa Berlaku Dokumen Aplikasi 
