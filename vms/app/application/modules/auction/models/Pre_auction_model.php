...
class Pre_auction_model extends CI_Model{
	...
	function initial_graph($id_lelang = "", $id_barang = ''){
		...
		return $this->db
			->select('a.*, b.nama')
			->from('ms_penawaran a')
			->join('ms_vendor b', 'a.id_vendor = b.id', 'left')
			->where('a.id_procurement', (int) $id_lelang)
			->where('a.id_barang', (int) $id_barang)
			->order_by('a.entry_stamp', $order)
			->limit(10, $start)
			->get();
	}
		...
		foreach($_barang->result() as $data){
			if($data->is_catalogue==1) {
				if($data->id_material==0){
					$this->db->insert('ms_material',array(
						'id_barang'=>$data->id,
						'nama'=>$data->nama_barang,
						'category'=>$data->category,
						'for'=>($data->category=='jasa') ? 6 : 1,
	...
	function vendor_experience($id_procurement){
   	...
   	function get_vendor_ranking($id_lelang = '', $id_barang = '', $type_lelang = ''){
		...
		return $this->db->query($sql, array((int) $id_barang, (int) $id_lelang, (int) $id_barang, (int) $id_barang))->result_array();
	}
}
