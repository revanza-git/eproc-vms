...
class Custom_report extends CI_Controller{
	...
	public function index(){
		...
		// print_r($data['pengadaan']);die;
		foreach($data['pengadaan'] as $key => $row){

			$data_kontrak 										= $this->pm->get_kontrak($row['id']);
			$contract_id										= isset($data_kontrak['id']) ? $data_kontrak['id'] : null;

			$data['pengadaan'][$key]['progress'] 				= $this->pm->get_paket_progress($row['id']);

			$data['pengadaan'][$key]['graph']					= ($contract_id) ? $this->pm->get_graph($contract_id,$row['id']) : array('data'=>array(),'total_day'=>0);
			...
			$last_pekerjaan										= 'Dalam masa '.(isset($pekerjaan['step']) ? $pekerjaan['step'] : '') .' pada hari ke-'.(isset($pekerjaan['now']) ? $pekerjaan['now'] : '0');
...
// echo $id;die;
		$data['pengadaan']		= $this->crm->get_pengadaan_progress_by_id($id);
		...
		echo(json_encode($data['pengadaan']));
	}
