...
class Utility{
	...
	function drop_down_vendor($name = '', $id_pengadaan = '', $selected = ''){
		...
		$sql = $this->CI->db->query(
			'SELECT b.id, b.nama, "is_vms" AS source
			 FROM ms_peserta_pengadaan a
			 LEFT JOIN ms_vendor b ON a.id_vendor = b.id
			 WHERE a.id_pengadaan = ?
			   AND b.id NOT IN (SELECT id_vendor FROM tr_blacklist WHERE id_pengadaan = ?)
			 UNION ALL
			 SELECT id, nama, "non_vms" AS source
			 FROM ms_peserta_non_vms
			 WHERE id_pengadaan = ? AND is_blacklist IS NULL',
			array($id_pengadaan, $id_pengadaan, $id_pengadaan)
		);
	...
	function drop_down_sbu($name = '', $selected = ''){
		...
		$this->CI->db->from('tb_sbu');
		$this->CI->db->where('id NOT IN (SELECT id_sbu FROM ms_admin)', null, false);
		if ($selected) {
			$this->CI->db->or_where('id', $selected);
		}
		$sql = $this->CI->db->get();
