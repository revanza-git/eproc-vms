...
- Wave B Execution Update: `February 26, 2026` (Stage 2 scoped route toggle + rollback switch validation + sibling bind-mount hook for final skeleton, plus post-merge `33139dc` revalidation with fallback placeholder)
- Scope Evidence:
...
| B2 | Resolved (Stage 2 scoped coexistence route toggle proven in dev; in-project Laravel skeleton integrated) | Service `pilot-app`, route shadow `/_pilot/auction/*`, route toggle subset `get_barang/get_peserta` + rollback switch (`CX-03`,`CX-04`) tervalidasi di dev via marker routing, hook `bind mount sibling` untuk skeleton Laravel final tetap siap via `EPROC_PILOT_APP_BIND_PATH`, dan next-step integrasi skeleton Laravel final-compatible ke path in-project `./pilot-app` sudah selesai tanpa merusak smoke Stage 1/2. | `docker-compose.yml`, `docker-compose.php82.yml`, `docker/nginx/default.conf`, `docker/nginx/includes/pilot-auction-subset-toggle.active.conf`, `docker/nginx/templates/pilot-auction-subset-toggle.legacy.conf`, `docker/nginx/templates/pilot-auction-subset-toggle.pilot.conf`, `pilot-app/routes/web.php`, `tools/dev-env.ps1`, `.env`, `.env.example`, `docs/PHASE6_COEXISTENCE_DEV_BASELINE.md`, `docs/DEV_ENV_RUNBOOK.md` | Coexistence infra dev untuk scope subset tetap siap dan kini source pilot default di `./pilot-app` sudah berupa skeleton Laravel dengan stub kompatibel. Hook `EPROC_PILOT_APP_BIND_PATH` tetap dipertahankan untuk opsi repo terpisah; domain migration nyata + auth bridge + CI gates tetap pending (B1/B4/B6) |
...
1. Lanjut dari skeleton Laravel in-project yang sudah terpasang di `./pilot-app`: pertahankan stub kompatibel sambil mulai implementasi endpoint pilot nyata secara bertahap; jika source dipindah ke repo terpisah, gunakan hook `EPROC_PILOT_APP_BIND_PATH` dan ulangi smoke `CX-01` s.d. `CX-04`.
