...
class K3_model extends CI_Model{
	...
	function update_score($post,$id){
		...
		$id_csms_limit = isset($post['id_csms_limit']) ? $post['id_csms_limit'] : $this->get_k3_limit($post['score']);
		$data_status = isset($post['data_status']) ? $post['data_status'] : 0;
	...
	function save_quest($data){
		...
		$param = array(
			isset($data['id_question']) ? $data['id_question'] : 0,
			isset($data['type']) ? $data['type'] : '',
			isset($data['value']) ? $data['value'] : '',
			isset($data['label']) ? $data['label'] : '',
			isset($data['entry_stamp']) ? $data['entry_stamp'] : date('Y-m-d H:i:s'),
			0
		);
