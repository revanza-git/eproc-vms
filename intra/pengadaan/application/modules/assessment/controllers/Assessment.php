...
class Assessment extends CI_Controller {
	...
	public function form_assessment($id,$id_vendor){
		...
		if($this->input->post('simpan')){
			...
			if($assessment){
				$text = 'Sukses menyimpan data assessment!';
	...
	public function print_bast($id, $id_vendor){
		...
		$besaran_kontrak_rupiah = 'Rp.'.number_format($pengadaan->besaran).'(<b>'.terbilang($pengadaan->besaran).' rupiah</b>)';
		$prefix = ($pengadaan->besaran>0) ? 'dan' : '';
		$besaran_kontrak_kurs 	= $prefix.' '.$pengadaan->nama_kurs.' '.number_format($pengadaan->besaran_kurs).'(<b>'.terbilang($pengadaan->besaran_kurs).' '.$pengadaan->nama_kurs.'</b>)';
		$data['text'] 		= str_replace(
								...
								array(
									...
									($pengadaan->besaran>0) ? $besaran_kontrak_rupiah : '',
									get_hari(date('Y-m-d')),
									date('j'),
									get_month(date('Y-m-d')),
									date('Y'),
									($pengadaan->besaran_kurs>0) ? $besaran_kontrak_kurs : '',
								),
								$data['text']
							);
	...
	public function export_bast($id,$id_vendor){
		...
		header("Content-Disposition: attachment; Filename=BAST ".$data['name'].".doc");
	...
	public function export_excel($year = null){
		if ($year === null) {
		...
		$sort = $this->utility->generateSort(array('ms_procurement.name', 'pemenang', 'point', 'tr_assessment.category'));
		$filter = (array) $this->input->post('filter');

		$data = $this->am->get_pengadaan_list($year, $search, $sort, '', '', FALSE, $filter);

		$title = 'Data Assessment ' . $year;
		$table = "<table border=1>";

			$table .= "<tr>";
			$table .= "<td style='background: #f6e58d;'>No</td>";
			$table .= "<td style='background: #f6e58d;'>Nama Pengaadaan</td>";
			$table .= "<td style='background: #f6e58d;'>Nama Pemenang Sesuai Kontrak</td>";
			$table .= "<td style='background: #f6e58d;'>Nomor Kontrak</td>";
			$table .= "<td style='background: #f6e58d;'>Nilai Pengadaan</td>";
			$table .= "<td style='background: #f6e58d;'>Nilai Kontrak</td>";
			$table .= "<td style='background: #f6e58d;'>Score Assessment</td>";
			$table .= "<td style='background: #f6e58d;'>Kategori</td>";
			$table .= "</tr>";
		...
		foreach ($data as $key => $value) {
			...
			$table .= "<td>".$value['pemenang']."</td>";
			$table .= "<td>".$value['no_contract']."</td>";
			$table .= "<td>Rp. ".$value['idr_value']."</td>";
			$table .= "<td>Rp. ".$value['contract_price']."</td>";
			$table .= "<td>".$value['point']."</td>";
