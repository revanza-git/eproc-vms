...
class blacklist extends CI_Controller {
	...
	public function tambah($id=0){
		...
		$this->form_validation->set_message('valid_vendor', 'Nama Penyedia Barang & Jasa harus dipilih dari daftar.');
		if($this->form_validation->run()==TRUE){
			...
			$blacklist_session = $this->session->userdata('blacklist');
			if (is_array($blacklist_session) && !empty($blacklist_session['site'])) {
	...
	public function valid_vendor($id_vendor){
		$id_vendor = trim((string)$id_vendor);
...
$blacklist = $data['id_blacklist'];			$_POST['id_blacklist'] = $id_blacklist;
	...
	public function whitelist(){
		...
		$data['whitelist']	= $this->bm->whitelist_data(array(), $sort, $page, $per_page, TRUE);
		// print_r($data['whitelist']);
		$data['pagination'] = $this->utility->generate_page('blacklist/whitelist',$sort, $per_page, $this->bm->whitelist_data(array(), $sort, '', '', FALSE));
		$data['sort'] 		= $sort;
	...
	public function export_whitelist($title="Data Whitelist"){
		$data = $this->bm->whitelist_data(array(), array(), '', 0, FALSE);
		// print_r($data);die;	
		$table = "<table border=1>";
		...
		foreach ($data as $key => $value) {
			...
			$table .= "<tr>";
			$table .= "<td>".$no."</td>";
			$table .= "<td>".(isset($value['name']) ? $value['name'] : '')."</td>";
			$table .= "<td>".(isset($value['start_date']) ? $value['start_date'] : '')."</td>";
			$table .= "<td>".(isset($value['end_date']) ? $value['end_date'] : '')."</td>";
			$table .= "<td>".(isset($value['total_bl']) ? $value['total_bl'] : '0')."</td>";
			$table .= "<td>".(isset($value['remark']) ? $value['remark'] : '')."</td>";
			$table .= "</tr>";
		}
		$table .= "</table>";
		header('Content-type: application/ms-excel');
