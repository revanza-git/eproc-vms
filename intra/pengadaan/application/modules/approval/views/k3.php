...
<div>
	<table class="tableData" style="margin-bottom: 20px">
		<tr>
			<td>
				Vendor : <?php echo (isset($legal_name) ? $legal_name : '').' '.(isset($name) ? $name : '');?>
			</td>
		...
		<tr>
			<td>
				Skor : <?php echo isset($get_csms['score']) ? $get_csms['score'] : '-';?>
			</td>
		</tr>
		<tr>
			<td>
				Kategori : <?php echo isset($get_csms['value']) ? $get_csms['value'] : '-';?>
			</td>
		</tr>
	</table>
	<form method="POST" enctype="multipart/form-data">
		...
		<div class="panel-group">
			...
			<?php 
			$total = 0;
			if (isset($evaluasi) && is_array($evaluasi)) {
			...
			} // end foreach evaluasi
			} // end if evaluasi isset
			?>
					<tr class="totalAllQuest">
						...
						<td><b><?php echo isset($get_csms['score']) ? $get_csms['score'] : '-';?></b></td>
					</tr>
					<tr class="totalAllQuest">
						<td colspan="5"></td>
						...
						<td><b><?php echo isset($get_csms['value']) ? $get_csms['value'] : '-';?></b></td>
					</tr>				</tbody>
            </table>
		...
		<div class="clearfix" style="text-align: right">
			<div style="margin-bottom: 20px">Tanggal terbit CSMS : <?php echo $this->form->calendar(array('name'=>'start_date','value'=>($this->form->get_temp_data('start_date'))?$this->form->get_temp_data('start_date'):(isset($get_csms['start_date']) ? $get_csms['start_date'] : '')), false);?></div>
			<label class="nephritisAtt">
				<input type="radio" name="status" value="1" <?php echo $this->data_process->set_yes_no(1, isset($score_k3['data_status']) ? $score_k3['data_status'] : '');?>>&nbsp;<i class="fa fa-check"></i>&nbsp;OK
			</label>
			<label class="pomegranateAtt">
				<input type="radio" name="status" value="0" <?php echo $this->data_process->set_yes_no(0, isset($score_k3['data_status']) ? $score_k3['data_status'] : '');?>>&nbsp;<i class="fa fa-times"></i>&nbsp;Not OK
			</label>
		</div>
