...
- Wave B Execution Update: `February 26, 2026` (Stage 2 scoped route toggle + rollback switch validation + sibling bind-mount hook for final skeleton, post-merge `33139dc` revalidation, Laravel read-only subset implementation for `get_barang/get_peserta`, nested read-only pilot implementation for `get_initial_data/get_chart_update` with graceful fallback, plus contract+compare harness baseline/artifact for subset `json_provider`)
- Scope Evidence:
...
Alasan: Wave A berhasil menurunkan blocker governance/documentation. Pada Wave B Stage 1/2, repo ini sudah membuktikan coexistence runtime dasar via service `pilot-app`, route shadow `/_pilot/auction/*`, serta route toggle subset endpoint bisnis `get_barang/get_peserta` dengan rollback switch cepat (Nginx include + reload) dan validasi `CX-01` s.d. `CX-04` untuk aspek routing/marker. Update terbaru menambahkan hook dev `bind mount sibling` (env-driven `EPROC_PILOT_APP_BIND_PATH`), menyelesaikan integrasi skeleton Laravel final-compatible langsung di path in-project `./pilot-app`, meng-upgrade endpoint subset pilot `get_barang/get_peserta` menjadi implementasi Laravel read-only berbasis query (dengan graceful degraded-empty fallback saat tabel dev `auction` tidak tersedia), melanjutkan endpoint nested read-only `get_initial_data/get_chart_update` dengan implementasi service/query builder + shadow-route verification (tanpa memperluas scoped toggle Stage 2 existing), dan menambahkan baseline contract+compare harness/artifact untuk subset `json_provider` agar compare dev yang terblokir tetap tercatat jujur sebagai `BLOCKED`. Coexistence ini diposisikan sebagai safety rail transisi menuju migrasi penuh CI3 -> Laravel, bukan endpoint akhir. Namun mandatory completion gate Phase 6 tetap belum terpenuhi (pilot migration belum dijalankan), auth bridge `CX-05` + contract/integration/UAT/rollback drill evidence CI masih pending, dan evidence rollback saat ini masih marker-based karena sample DB dev CI3 belum memiliki tabel data `auction` yang dibutuhkan.
...
|---|---|---|---|
| Finalisasi endpoint inventory `auction` read-only (path-by-path + contract ringkas + auth assumption) | Resolved (v1 scope) | `docs/PHASE6_DECISION_RECORDS.md` (appendix Wave B) | Scope v1 dikunci untuk `vms` host; nested runtime sample CI3 masih pending (minimum contract baseline subset kini sudah dibekukan) |
...
| Re-validasi Stage 1/2 pasca-merge hook sibling bind mount (`33139dc`) | Resolved (stabil dengan fallback placeholder; final sibling hookup blocked) | `docs/PHASE6_COEXISTENCE_DEV_BASELINE.md`, `docs/DEV_ENV_RUNBOOK.md` | `CX-01`..`CX-04` re-run PASS dengan `EPROC_PILOT_APP_BIND_PATH=./pilot-app`; repo sibling Laravel final masih belum tersedia/terdeteksi |
| Integrasi skeleton Laravel final-compatible ke path in-project `./pilot-app` (keputusan terbaru) + re-run smoke | Resolved (next-step Wave B completed) | `pilot-app/artisan`, `pilot-app/composer.json`, `pilot-app/routes/web.php`, `docs/PHASE6_COEXISTENCE_DEV_BASELINE.md`, `docs/DEV_ENV_RUNBOOK.md` | `./pilot-app` kini berisi skeleton Laravel (Laravel 8, PHP 7.4-compatible) dengan stub route kompatibel + marker `X-App-Source: pilot-skeleton`; `docker compose config`, `coexistence`, `coexistence-stage2` tetap PASS; `CX-04` marker rollback CI3 PASS dengan HTTP `500` accepted (gap tabel dev) |
| Implement endpoint pilot subset `auction` read-only nyata di Laravel (`get_barang`, `get_peserta`) + revalidasi coexistence | Resolved (implementation done; dev data path degraded gracefully) | `pilot-app/routes/web.php`, `pilot-app/app/Http/Controllers/PilotAuctionController.php`, `pilot-app/app/Services/Auction/JsonProviderReadOnlyService.php`, `docker-compose.yml`, `docs/PHASE6_COEXISTENCE_DEV_BASELINE.md`, `docs/DEV_ENV_RUNBOOK.md` | Endpoint pilot tidak lagi hardcoded stub; query builder path/logging/headers degradasi ada. `CX-01`..`CX-04` tetap PASS; pilot ON response sample `[]` karena tabel dev `eproc.ms_procurement_barang` / `eproc.ms_procurement_peserta` tidak tersedia (accepted, no seed requirement added) |
| Implement endpoint pilot nested read-only `auction` berikutnya (`get_initial_data`, `get_chart_update`) + verifikasi aman via shadow route | Partial (query path implemented; graceful fallback proven; runtime compare CI3 sample blocked by dev schema gap) | `pilot-app/routes/web.php`, `pilot-app/app/Http/Controllers/PilotAuctionController.php`, `pilot-app/app/Services/Auction/JsonProviderReadOnlyService.php`, `docs/PHASE6_COEXISTENCE_DEV_BASELINE.md`, `docs/DEV_ENV_RUNBOOK.md`, `docs/REVAMP_CHECKLIST.md` | Scoped toggle Stage 2 tetap hanya `get_barang/get_peserta`; shadow route digunakan untuk verifikasi nested. Pilot nested response fallback object-shape `200 OK` terbukti; sample CI3 nested runtime masih `500` di dev sehingga compare payload penuh belum tersedia |
| Bekukan contract + compare harness subset `auction/admin/json_provider` (`get_barang`, `get_peserta`, `get_initial_data`, `get_chart_update`) | Resolved (baseline frozen; follow-up sample hunt tetap blocked tercatat granular) | `tools/compare-auction-json-provider-subset.ps1`, `docs/artifacts/phase6-waveb-json-provider-subset-compare-20260226-124453.json`, `docs/artifacts/phase6-waveb-json-provider-subset-compare-20260226-130218.json`, `docs/PHASE6_COEXISTENCE_DEV_BASELINE.md`, `docs/DEV_ENV_RUNBOOK.md` | Harness reusable merekam `PASS/BLOCKED/MISMATCH`; follow-up sample hunt masih `BLOCKED` untuk keempat endpoint karena CI3 dev `HTTP 500` dan inspeksi schema lokal menunjukkan tabel runtime subset tidak tersedia (bukan false PASS). Cutover bisnis tetap belum ready (auth/CI/UAT pending) |
...
| B2 | Resolved (Stage 2 scoped coexistence route toggle proven in dev; in-project Laravel skeleton integrated + subset read-only implemented) | Service `pilot-app`, route shadow `/_pilot/auction/*`, route toggle subset `get_barang/get_peserta` + rollback switch (`CX-03`,`CX-04`) tervalidasi di dev via marker routing, hook `bind mount sibling` untuk skeleton Laravel final tetap siap via `EPROC_PILOT_APP_BIND_PATH`, next-step integrasi skeleton Laravel final-compatible ke path in-project `./pilot-app` selesai, dan endpoint subset pilot sudah di-upgrade ke implementasi Laravel read-only query-based dengan graceful degraded-empty fallback untuk gap tabel dev. | `docker-compose.yml`, `docker-compose.php82.yml`, `docker/nginx/default.conf`, `docker/nginx/includes/pilot-auction-subset-toggle.active.conf`, `docker/nginx/templates/pilot-auction-subset-toggle.legacy.conf`, `docker/nginx/templates/pilot-auction-subset-toggle.pilot.conf`, `pilot-app/routes/web.php`, `pilot-app/app/Http/Controllers/PilotAuctionController.php`, `pilot-app/app/Services/Auction/JsonProviderReadOnlyService.php`, `tools/dev-env.ps1`, `.env`, `.env.example`, `docs/PHASE6_COEXISTENCE_DEV_BASELINE.md`, `docs/DEV_ENV_RUNBOOK.md` | Coexistence infra dev untuk scope subset tetap siap dan kini source pilot default di `./pilot-app` sudah berupa skeleton Laravel dengan endpoint subset query-based (bukan stub hardcoded). Hook `EPROC_PILOT_APP_BIND_PATH` tetap dipertahankan untuk opsi repo terpisah; data sample dev masih gap (degraded fallback) dan domain migration/auth bridge/CI gates tetap pending (B1/B4/B6) |
| B3 | Resolved (Wave B v1 pilot scope locked) | Inventory endpoint `auction` read-only sudah dipublikasikan path-by-path (v1, host `vms`) beserta contract ringkas, pilot subset decision, dan backlog deferred endpoint. Minimum shape subset (`get_barang`, `get_peserta`, `get_initial_data`, `get_chart_update`) kini sudah dibekukan sebagai baseline compare/cutover-readiness; compare runtime CI3 vs pilot penuh masih terblokir oleh gap schema dev. | `docs/PHASE6_DECISION_RECORDS.md`, `docs/FRAMEWORK_MIGRATION_STRATEGY.md`, `docs/REVAMP_CHECKLIST.md`, `docs/PHASE6_COEXISTENCE_DEV_BASELINE.md`, `tools/compare-auction-json-provider-subset.ps1` | Ambil sample payload runtime CI3 (anonymized) saat tabel/data dev tersedia, lalu finalize contract snapshot untuk `pilot-contract` CI |
...
| B6 | Partial | Rencana gate pilot contract/integration + template evidence UAT/rollback sudah tersedia, ditambah draft contract matrix/integration cases, helper local `coexistence`/`coexistence-stage2` untuk proof route shadow + toggle rollback di dev, dan harness compare subset `json_provider` untuk artifact `PASS/BLOCKED/MISMATCH` lokal (termasuk follow-up sample hunt granular). | `docs/CI_QUALITY_GATES.md`, `docs/PHASE6_DECISION_RECORDS.md`, `tools/dev-env.ps1`, `tools/compare-auction-json-provider-subset.ps1`, `docs/templates/PHASE6_PILOT_UAT_EVIDENCE_TEMPLATE.md`, `docs/templates/PHASE6_PILOT_ROLLBACK_DRILL_TEMPLATE.md`, `docs/artifacts/phase6-waveb-json-provider-subset-compare-20260226-124453.json`, `docs/artifacts/phase6-waveb-json-provider-subset-compare-20260226-130218.json` | Workflow/job CI pilot (`pilot-contract`, `pilot-integration`, evidence checks) dan run PASS belum ada |
...
1. Lanjut dari skeleton Laravel in-project yang sudah terpasang di `./pilot-app`: pertahankan compatibility marker/toggle sambil melanjutkan endpoint pilot nyata berikutnya (nested `get_initial_data/get_chart_update` sudah query-based via shadow verification; endpoint berikutnya di luar itu); jika source dipindah ke repo terpisah, gunakan hook `EPROC_PILOT_APP_BIND_PATH` dan ulangi smoke `CX-01` s.d. `CX-04`.
2. Ambil sample payload anonymized CI3 runtime valid (`HTTP 200`) untuk subset `json_provider` dan rerun `tools/compare-auction-json-provider-subset.ps1` agar hasil compare naik dari `BLOCKED` ke `PASS/MISMATCH` berbasis payload nyata; catatan follow-up terbaru menunjukkan DB lokal tidak memiliki tabel runtime subset `auction`, jadi source sample perlu environment dev/staging/non-prod yang aman (tanpa seed data baru) untuk finalize schema nested `pilot-contract`.
...
5. Resolved (2026-02-26): path dev yang dipakai berada di dalam project ini (`./pilot-app`). Hook `EPROC_PILOT_APP_BIND_PATH` tetap dipertahankan jika nanti dipindahkan ke repo terpisah.
