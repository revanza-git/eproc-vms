...
class evaluasi_model extends CI_Model{
	...
	function get_evaluasi_list($search='', $sort='', $page='', $per_page='',$is_page=FALSE,$filter=array()){
		$this->db->select('*');
		$this->db->where('del',0);
    ...
    public function save_evaluasi($data){
		...
		$param = array(
			isset($data['id_ms_quest']) ? $data['id_ms_quest'] : 0,
			isset($data['name']) ? $data['name'] : '',
			isset($data['point_a']) ? $data['point_a'] : 0,
			isset($data['point_b']) ? $data['point_b'] : 0,
			isset($data['point_c']) ? $data['point_c'] : 0,
			isset($data['point_d']) ? $data['point_d'] : 0,
			0
		);
