...
class Filter{
	...
	public function group_filter_post($field=null){
		...
		$filter = (array) $this->CI->input->post('filter');
					...
					foreach($field as $row){
						...
						$html .= '<div class="groupForm">
									...
									foreach($row['filter'] as $key => $value){
										...
										$tableParts = explode('|', $value['table']);
										...
										switch ($value['type']) {
											case 'text':
												...
												if(isset($filter[$value['table']]) && is_array($filter[$value['table']])){
													foreach($filter[$value['table']] as $key => $row){
														$class_attr = isset($value['class']) ? $value['class'] : '';
														$html .= '<input type="text" name="filter['.$value['table'].'][]" value="'.$row.'" class="'.$class_attr.'">';
													}
												}
											...
											case 'dropdown':
												$html .= '<div class="groupFieldBlock"><label class="title">'.$value['label'].'</label>';
												$callback = isset($tableParts[2]) ? $tableParts[2] : null;
												$data_dropdown = array();
												if (is_string($callback) && $callback !== '' && method_exists($this, $callback))
												{
													$data_dropdown = $this->{$callback}();
												}
											$html .= '<div class="groupFieldInput" name="filter['.$value['table'].'][]" >'.
											...
											if(isset($filter[$value['table']]['start_date']) && is_array($filter[$value['table']]['start_date'])){
												...
												$html.='</div>';
												$html .= '<div class="plusMinusWp"><a href="#" class="addFilterGroupDate" ><i class="fa fa-plus"></i></a>&nbsp;<a href="#" class="removeFilterGroupDate" ><i class="fa fa-minus"></i></a></div></div></div>';
												$html .= '</div>';
												break;
		...
		$html .='</div><button style="margin: 0 0 35px;" type="submit" class="btnBlue"><i class="fa fa-filter"></i>&nbsp;Filter</button></div></div>';
	...
	public function generate_query($instance,$filter){
		...
		$filter_session_data = $this->CI->session->userdata('filter');
		$sess_filter = isset($filter_session_data[$this->CI->uri->uri_string()]) ? $filter_session_data[$this->CI->uri->uri_string()] : array();

		if(is_array($post_filter)){
			foreach($post_filter as $key => $val_arr){
				if(is_array($val_arr)){
					if(count($val_arr)==1 && isset($val_arr[0]) && $val_arr[0]=='' )
		...
		$updated_filter_session = $this->CI->session->userdata('filter');
		$filter_data = isset($updated_filter_session[$this->CI->uri->uri_string()]) ? $updated_filter_session[$this->CI->uri->uri_string()] : array();
