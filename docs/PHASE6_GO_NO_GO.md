...
| B3 | Resolved (Wave B v1 pilot scope locked) | Inventory endpoint `auction` read-only sudah dipublikasikan path-by-path (v1, host `vms`) beserta contract ringkas, pilot subset decision, dan backlog deferred endpoint. Minimum shape nested (`get_initial_data`, `get_chart_update`) kini sudah direkam dari CI3 query path + hasil runtime pilot fallback; compare runtime CI3 vs pilot penuh masih terblokir oleh gap schema dev. | `docs/PHASE6_DECISION_RECORDS.md`, `docs/FRAMEWORK_MIGRATION_STRATEGY.md`, `docs/REVAMP_CHECKLIST.md`, `docs/PHASE6_COEXISTENCE_DEV_BASELINE.md` | Ambil sample payload runtime CI3 (anonymized) saat tabel/data dev tersedia, lalu finalize contract snapshot untuk `pilot-contract` CI |
...
2. Ambil sample payload anonymized CI3 runtime untuk `get_initial_data` dan `get_chart_update` (minimum shape sudah direkam dari query path + pilot fallback), lalu finalize schema nested untuk `pilot-contract`.
