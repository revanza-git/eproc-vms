...
<div class="dataWrapper col-24">
	   	...
	   	<div class="tableWrapper">
			<table class="tableData">
				...
				if(count($chart['daftar_tunggu_chart']->result_array())){
					...
					?>
						<form method="POST" action="<?php echo site_url('admin/admin_vendor/waiting_list')?>">
							<tr>
			...
			<div class="buttonRegBox clearfix">
				<a href="<?php echo site_url('admin/admin_vendor/waiting_list');?>" class="btnBlue">Lihat Daftar Tunggu</a>
			</div>
		</div>
	</div>
	<?php } 
	...
	<?php } ?>
	<?php if($this->session->userdata('admin')['id_role']!= 2){;?>
	<?php if(count($chart['daftar_hitam_chart']->result_array())){ ?>
	<div class="block">
	    <h4>Daftar Hitam</h4>
	   	<div class="tableWrapper">
			<table class="tableData">
				...
				<tbody>
				<?php 
				if(count($chart['daftar_hitam_chart']->result_array())){
					foreach($chart['daftar_hitam_chart']->result_array() as $row => $value){
					?>
						<tr>
							<td><?php echo $value['name'];?></td>
							<td class="actionBlock">
								...
								<?php }
									...
									if($this->session->userdata('admin')['id_role']==8){
										...
										if($value['need_approve_bl']==1){
											if($value['is_white']==1){ ?>
											| <a href="<?php echo site_url('blacklist/approve_aktif/'.$value['id'])?>" ><i class="fa fa-check-square-o"></i>Setuju Aktifkan kembali</a>
											<?php } else{ ?>
											| <a href="<?php echo site_url('blacklist/approve_form/'.$value['id_vendor_bl'].'/'.$value['id_tr'].'/'.$value['id_blacklist'])?>"><i class="fa fa-check-square-o"></i>&nbsp;Setujui Masuk <?php echo $value['blacklist_val']?></a>
											<?php }
										}
									...
									if($this->session->userdata('admin')['id_role']==1 && $value['need_approve_bl'] == 0){
										?>
										| <a href="<?php echo site_url('blacklist/aktif/'.$value['id'])?>" ><i class="fa fa-check-square-o"></i>Aktifkan</a>
										<?php
									}
			...
			<div class="buttonRegBox clearfix">
				<a href="<?php echo site_url('blacklist/index/2');?>" class="btnBlue">Lihat Daftar Hitam</a>
			</div>
		</div>
	</div>
	<?php }
	...
	<div class="block">
	    <h4>Daftar Merah</h4>
	   	<div class="tableWrapper">
			...
			<div class="buttonRegBox clearfix">
				<a href="<?php echo site_url('blacklist/index/1');?>" class="btnBlue">Lihat Daftar Merah</a>
			</div>
		</div>
	</div>
	<?php }
	...
	<div class="block">
	    <h4>DPT</h4>
	   	<div class="tableWrapper">
			...
			<div class="buttonRegBox clearfix">
				<a href="<?php echo site_url('admin/admin_dpt');?>" class="btnBlue">Lihat DPT</a>
			</div>
		</div>
	</div>
	<?php } ?>
