					...
					foreach($field as $row){
						...
						$html .= '<div class="groupForm">
									...
									foreach($row['filter'] as $key => $value){
										...
										$table_parts = explode('|',$value['table']);
										...
										switch ($value['type']) {
											...
											case 'dropdown':
												$html .= '<div class="groupFieldBlock"><label class="title">'.$value['label'].'</label>';
												$dropdown_method = isset($table_parts[2]) ? $table_parts[2] : NULL;
												$data_dropdown = ($dropdown_method !== NULL && method_exists($this, $dropdown_method))
													? $this->{$dropdown_method}()
													: array();
												$html .= '<div class="groupFieldInput" name="filter['.$value['table'].'][]" >'.
